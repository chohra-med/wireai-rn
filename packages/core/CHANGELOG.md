...
### Changed
- **Three shipped component behaviours are now covered by a mount-based React test harness.** `packages/core` carried no React renderer at all, so nothing that renders a component could be tested, and the `isStreaming` tap guard, the background interruption with its `retry()` path, and the submitted disabled visual were each verified only by reading the code. The package gains `react-test-renderer` and `@types/react-test-renderer` as devDependencies, a harness plus headless React Native host mocks under `src/__tests__/harness/`, and 19 tests in three new suites covering those three behaviours. The harness lives inside a path the `files` field already excludes, so none of it reaches the npm tarball. One jest change came with it: the ts-jest transform now compiles JSX to `React.createElement` rather than preserving it, because this package tsconfig targets Metro and node cannot parse preserved JSX. `testMatch` is untouched and every pre-existing test file is still collected. No runtime source file changed, no public API changed, and `dist/index.d.ts` is byte identical to the build from the commit this branched from.

### Fixed
- **`abort()` and `reset()` no longer swallow the next `sendMessage` or `retry()`.** Both lowered the `isLoading` state without lowering `isLoadingRef`, and the guards inside `sendMessage` and `retry()` read that ref rather than the state. The ref stays true until the rejection from the request they just aborted reaches `.finally()`, so a consumer who stopped a turn and immediately sent the next message, or reset the thread and started it again, got nothing back and no reason why. Both now clear the ref alongside the state, the same way the `AppState` background handler does.
- **`retry()` is no longer a silent no-op right after the app is backgrounded.** The `AppState` handler cleared the `isLoading` state but not `isLoadingRef`, and `retry()` guards on the ref. The ref stays true until the aborted request's rejection reaches `.finally()`, so between backgrounding and that rejection landing, the retry the `"interrupted"` state exists to offer returned without doing anything and without saying why. The handler now clears the ref alongside the state.
- **A submitted chip, item or option row looks submitted.** The disabled visual added for the streaming gate fired on `isStreaming` only, while the `toggle` handler behind it has always returned early on `submitted || isStreaming`. After a submit the rows therefore still rendered as live controls that silently ignored a tap. The three row components (`ChipSelectCard`, `ContentSelectCard`, `SelectionCard`) now take the disabled visual on either condition, matching their own handlers and the submit button beside them.
...
- **Interactive built-in components are disabled while their message is still streaming.** `ComponentRenderer` has always injected `isStreaming` into every rendered component, but no built-in consumed it, so every button, chip, option row and text field of a partially rendered component was live mid-stream. A tap fired a handler whose result was then overwritten by the next chunk or by the final validated render, with no feedback that it had done nothing. All ten interactive built-ins now gate on the injected value, and chip, item and option rows gained a disabled visual state they never had. `MessageBubble` is display only and is unchanged. No schema, description, prop or export changed. Read this as a capability, not a guarantee: `ComponentRenderer` defaults `isStreaming` to `false`, so the flag only reaches the built-ins if the consumer's own `ComponentRenderer` call passes it. A screen that renders without it behaves exactly as before. The README and DOCUMENTATION quick starts now pass it.
- **Documentation corrected against the code.** The SDK README had the streaming matrix backwards: it named OpenAI and Webhook as the only adapters implementing `chatStream` and listed Ollama and LM Studio as one-shot, when A2A is in fact the only adapter without `chatStream`. The README's "How It Works" diagram and the repo landing README both routed invalid model output to `FallbackMessage`, which that path never reaches: the validator throws, the thread hook drops the turn and sets `error`, and `FallbackMessage` covers per node failures inside the renderer and a render crash caught by the error boundary. Both are now described where they actually happen. The streaming section and the provider setup section also state plainly that the SDK has no offline or cold-start lane, and the repo landing README's Wire AI attribution line now matches the current product wording. 2026-08-19, second pass: `src/hooks/README.md` ships inside the npm tarball and its `useWireAIThread` row and destructuring block still predated `errorKind` and `retry`, so both are now listed and explained; `DOCUMENTATION.md` left A2A out of the adapter enumeration in its architecture diagram and never mentioned `isStreaming` at all, so the adapter list is complete and section 12 now says what the flag does. The `isStreaming` docstrings on `InjectedProps` and `ComponentRenderer` name both of the flag's effects instead of calling it an optional UI hint, and the `LocalLLMConfigSchema` header no longer promises that anything a consumer passes survives storage when `z.object` strips undeclared keys.
- **`FEATURES.md` no longer names an API that does not exist.** The file is copied into the marketing website by its prebuild, so every false identifier in it became public copy. Corrected in two passes. 2026-08-18: its hook list named three hooks the SDK does not export and its provider was written without the `AI` that is in the real export name, so the list now matches `src/index.ts`, and the version line points at `packages/core/package.json` instead of carrying a literal that had been stale since 0.1.3; the two dated release notes claiming that every adapter streams keep their text and carry an inline correction. 2026-08-19: the remaining false claims went too. The file sold a theming API three times over (`Themeable via WireAIProvider`, `Theme tokens` in the provider's exposed list, and "consumers can override tokens via `WireAIProvider` props"), and no such prop exists: `styles/tokens.ts` is frozen constants and the provider takes `llm`, `components`, `maxContextMessages`, `maxContextChars`, `systemPromptSuffix`, `initialMessages`, `onMessage`, `onThreadUpdate`, `licenseKey`, `streaming` and `children`. The tokens are exported and reusable, which is what the file now says. Its streaming line also named an "XHR ReadableStream polyfill" that does not exist (the adapters read `XMLHttpRequest.onprogress` directly, no polyfill) and stated streaming-by-default without the A2A carve-out. No behaviour changed in either pass.
