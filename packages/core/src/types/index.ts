...
export type Message = {
   ...
   * that became `content` itself — uninterpreted, in the producing adapter's
   * collection order. Today only `A2AAdapter` produces it, from an A2A task
   * whose agent message holds more than one DataPart; it gathers parts across
   * the whole task (agent messages latest-first, then artifacts latest-first),
   * so wire order holds only within a single message's own parts array.
   *
   * The SDK deliberately does not know what any of these payloads mean: it is
   * a generic carrier, so a consumer narrows by its own convention — by a
   * marker the payload carries, never by index (for the Wire onboarding
   * server, a part with `kind: "onboarding_plan"`). Absent — not `[]` — when
   * the turn carried no extra data.
   */
  dataParts?: unknown[];
